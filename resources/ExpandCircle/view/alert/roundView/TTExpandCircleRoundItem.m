...
//    [Growing track:GIO_KuoquanGesture withVariable:@{GIO_kGestureType: @"喜欢"}];
    if (!self.dispalyLink) {
...
//    [Growing track:GIO_KuoquanGesture withVariable:@{GIO_kGestureType: @"不喜欢"}];
    if (!self.dispalyLink) {
        self.dispalyLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(disAppearGoing)];
        self.disAppearFlag = -15;
        [self.dispalyLink addToRunLoop:[NSRunLoop currentRunLoop] forMode:NSRunLoopCommonModes];
...
- (void)photoCoosedAtIndex:(NSInteger)index withItems:(NSArray *)items {
    if ([self.delegate respondsToSelector:@selector(expandUser:PhotoClickedAtIndex:fromViews:)]) {
        [self.delegate expandUser:self.infoView.user PhotoClickedAtIndex:index fromViews:items];
    }
}
